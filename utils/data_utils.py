import random
from utils.shot_utils import Get_shot_score, Get_shot_classes, split_shot
import h5py


def load_data(args):
    print("‘\n------------>读取数据<--------------")
    keys, feature_dic, labels_score_dic,
    shot_score_dic, shot_classes_dic = Read_data(args.Dataset_root, args.dataset)
        split_key = data_split(keys, args.split_seed)
        return split_key, feature_dic, labels_score_dic, nfps_dic, positions_dic, user_summary_dic, shot_score_dic, shot_classes_dic
