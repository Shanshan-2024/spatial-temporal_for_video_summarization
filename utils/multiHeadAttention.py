...
from utils.utils import generate_local_map_mask
...
class MultiHeadAttentionWindow(MultiHeadAttention):
    ...
    def forward(self,
                ...
                mask: Optional[str] = None) -> torch.Tensor:
        '''
        :param query: [8, 10, 34, 128]
        :param key: [8, 10, 34, 128]
        :param value: [8, 10, 34, 128]
        :param mask:
        attention :[1360, 2, 128]->[8, 170, 2, 128] -> [1, 170, 2, 128] -> [1, 170, 2, 1024]
        :return:
        '''
        ...
        queries = queries.reshape(-1, self._q, queries.shape[-2]).transpose(1, 2)
        keys = keys.reshape(-1, self._q, queries.shape[-2]).transpose(1, 2)
        values = values.reshape(-1, self._q, queries.shape[-2]).transpose(1, 2)

        # Scaled Dot Product
        self._scores = torch.bmm(queries, keys.transpose(1, 2))
        ...
        # Fold chunks back
        attention = attention.reshape((batch_size*self._h, -1, values.shape[-2], self._v))
