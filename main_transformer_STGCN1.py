...
class Object():
    def __init__(self, args, model=None, k=None):
        ...
        self.split_key, self.feature_dic,  \
        ...
        for epoch in range(args.epochs):
            ...
            for video_index in self.train_keys:
                ...
                if args.Generate_adj == True:
                    ...
                    torch.save(Laplas_list, 'data/adj/{}_{}_Laplas_fragment_cosine_similarity'.format(args.dataset, video_index))
                else:
                    Adj_list = torch.load('data/adj/{}_{}_Adjacency_fragment_cosine_similarity'.format(args.dataset, video_index))
                    Laplas_list = torch.load('data/adj/{}_{}_Laplas_fragment_cosine_similarity'.format(args.dataset, video_index))

                    # output, h = self.model(video_feature, adj.to(self.device), 10)
                output, h = self.model(video_feature, adj.to(self.device), laplas_adj.to(self.device), 10)
