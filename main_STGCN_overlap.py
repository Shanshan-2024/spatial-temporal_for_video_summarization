...
from models.Transf_STGCN1 import Transf_STGCN
args = Get_args()
...
class Object():
    def __init__(self, args, model=None, k=None):
        ...
        if model == None:
            ...
            self.model = Transf_STGCN(num_features=args.inchannel, args=args).to(self.device)
        ...
        for epoch in range(args.epochs):
            ...
            for video_index in self.train_keys:
                ...
                for i in range(total_groups):
                    ...
                    video_feature.append(feature[start:end])
                video_feature = np.array(video_feature)  #[34, 12, 1024]
                video_feature = torch.tensor(video_feature)
                video_feature = video_feature.unsqueeze(0).to(self.device)
                video_feature = video_feature.permute(0, 2, 1, 3)  # [1, 12, 34, 1024]
                # video_feature = video_feature.permute(0, 2, 1, 3)  #[1, 34, 10, 1024] -> [1, 10, 34, 1024]
                # 是否设置生成邻接矩阵
                if args.Generate_adj == True:
                    from utils.GNN_utils import Generate_fragment_Adjacency_PE
                    Adj_list, Laplas_list = Generate_fragment_Adjacency_PE(fragment_feature=fragment_feature, args=args)
                    torch.save(Adj_list, 'data/adj/{}_{}_Adjacency_fragment_cosine_similarity_12'.format(args.dataset, video_index))
                    torch.save(Laplas_list, 'data/adj/{}_{}_Laplas_fragment_cosine_similarity_12'.format(args.dataset, video_index))
                else:
                    Adj_list = torch.load('data/adj/{}_{}_Adjacency_fragment_cosine_similarity_12'.format(args.dataset, video_index))
                    Laplas_list = torch.load('data/adj/{}_{}_Laplas_fragment_cosine_similarity_12'.format(args.dataset, video_index))

                for index, (adj, laplas_adj) in enumerate(zip(Adj_list, Laplas_list)):
                    # output, h = self.model(video_feature, adj.to(self.device), 10)
                    output, h = self.model(video_feature, adj.to(self.device), laplas_adj.to(self.device), 12)
